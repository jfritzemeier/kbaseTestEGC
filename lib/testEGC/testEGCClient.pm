...
sub new
{
    ...
    my $self = {
	client => testEGC::testEGCClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a testEGC.FilterContigsParams
$output is a testEGC.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	assembly_input_ref has a value which is a testEGC.assembly_ref
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	assembly_output has a value which is a testEGC.assembly_ref
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for testEGC::testEGCClient\n";
    }
    if ($sMajor == 0) {
        warn "testEGC::testEGCClient version is $svr_version. API subject to change.\n";
    }
}
...
package testEGC::testEGCClient::RpcClient;
